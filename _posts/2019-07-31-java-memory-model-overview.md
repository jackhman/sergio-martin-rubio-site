...
Code Cache is the memory area where the JVM stores the compiled code generated by the Just-In-Time (JIT) compiler. 

The momory used by Code Cache is 

Features:

- Code Cache is used to store the compiled code generated by the Just-in-time (JIT) compilers.
