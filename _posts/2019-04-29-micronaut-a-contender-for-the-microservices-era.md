...
>GraalVM: it is an ecosystem which support many languages and allows you to run your applications like a native one, so in the case of java it will translate your bytecode into machine code.
...
Test are also created in a similar way as **Spring Boot** way. Instead of 
`@SpringBootTest` annotation, it uses `@MicronautTest`, and `RxHttpClient` annotated with `@Client` instead of autowiring `TestRestTemplate` to make the HTTP requests to your controller.
...
**Micronaut** supports a number of different format for your application properties like application.yaml, application.properties or application.groovy.
...
**Micronaut only supports Netty** and **there is no way at the moment to switch to Tomcat**, so you have to considerer if the metrics and threading handling of Tomcat is a must. However, it is compatible with **Micrometer**, **provides refresh and healthcheck endpoints, routes and cache endpoints**.
...
This framework is also a good choice if you are starting a project from scratch.

## Caveats

- It is still very new, and the community is quite small compare to the Spring one, though big enterprise companies like ThoughtWorks has it already on their tech radar.
